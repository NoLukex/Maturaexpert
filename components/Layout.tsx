...
const Layout: React.FC<LayoutProps> = ({ currentView, onChangeView, children }) => {
  ...
  const NavItem = ({ view, icon: Icon, label }: { view: ViewState; icon: any; label: string }) => (
    <button
      ...
      className={`w-full flex items-center space-x-3 px-4 py-3.5 rounded-2xl transition-all mb-1.5 group relative overflow-hidden ${
        currentView === view 
    ...
    >
      ...
      <Icon size={20} className={`relative z-10 transition-transform duration-300 group-hover:scale-110 group-active:scale-95 ${currentView === view ? 'text-matura-bg' : 'text-gray-400 group-hover:text-white'}`} />
      ...
      {notification && (
        <div className="fixed top-6 right-6 z-[100] animate-slide-up">
          <div className="bg-[#112240]/90 backdrop-blur-xl border border-matura-accent/50 shadow-[0_0_30px_rgba(245,197,24,0.2)] rounded-2xl p-4 flex items-center gap-4 min-w-[320px] relative overflow-hidden">
             ...
             <div>
               <p className="text-matura-accent font-bold text-[10px] uppercase tracking-widest mb-0.5">Odblokowano Osiągnięcie!</p>
        ...
        <nav className="flex-1 px-6 py-4 space-y-1 overflow-y-auto custom-scrollbar">
          ...
          <div className="my-8">
            ...
            <div className="space-y-1">
              <NavItem view="vocabulary" icon={BookA} label="Słownictwo" />
              <NavItem view="grammar" icon={Library} label="Gramatyka" />
              <NavItem view="listening" icon={Headphones} label="Słuchanie" />
              <NavItem view="reading" icon={FileText} label="Czytanie" />
              <NavItem view="writing" icon={PenTool} label="Pisanie (AI)" />
              <NavItem view="speaking" icon={BrainCircuit} label="Matura Ustna" />
            </div>
          </div>
          ...
          <div className="my-8">
             ...
             <NavItem view="exam" icon={GraduationCap} label="Arkusze Maturalne" />
             <NavItem view="mistakes" icon={AlertOctagon} label="Bank Błędów" />
          </div>
        </nav>

        <div className="p-6 border-t border-white/5 bg-black/10">
          <NavItem view="settings" icon={Settings} label="Ustawienia" />
        ...
        {mobileMenuOpen && (
          <div className="fixed inset-0 z-40 bg-[#050B14]/95 backdrop-blur-xl pt-24 px-6 animate-fade-in">
             <nav className="space-y-2">
              ...
              <NavItem view="speaking" icon={BrainCircuit} label="Matura Ustna" />
              <NavItem view="mistakes" icon={AlertOctagon} label="Bank Błędów" />
              <NavItem view="exam" icon={GraduationCap} label="Arkusze" />
              <NavItem view="settings" icon={Settings} label="Ustawienia" />
            </nav>
          </div>
        )}
